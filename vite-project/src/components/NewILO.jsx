...
const NAV_ITEMS = [
  ...
  { label: "Create ILO", icon: "+", active: true },
];
...
function BlockModal({ value, onSelect, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center" }}
      onClick={onClose}>
      <div style={{ background: "#131920", border: "1px solid #1e2a3a", borderRadius: 12, padding: "24px", minWidth: 320 }}
        onClick={e => e.stopPropagation()}>
        <div style={{ fontSize: 15, fontWeight: 700, color: "#e8edf5", marginBottom: 16 }}>Block number</div>
        <input value={val} onChange={e => setVal(e.target.value)} style={{ width: "100%", background: "#0d1017", border: "1px solid #1e2a3a", borderRadius: 8, padding: "10px 12px", color: "#e8edf5", fontSize: 14, fontFamily: "inherit", outline: "none", boxSizing: "border-box" }} />
        <div style={{ fontSize: 11, color: "#4a5568", marginTop: 6 }}>Current block: 101990582</div>
        <button onClick={() => { onSelect(val); onClose(); }}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)" }}>
      <div style={{ background: "#111318", border: "1px solid #1a2030", borderRadius: 16, padding: "40px", width: 420, display: "flex", flexDirection: "column", alignItems: "center", gap: 18, textAlign: "center" }}>
        ...
        <button onClick={onClose} style={{ width: "100%", background: "linear-gradient(90deg,#a78bfa,#60a5fa,#34d399)", border: "none", borderRadius: 10, padding: "12px", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
          Done — View My ILO
        </button>
      </div>
    </div>
...
export default function CreateILO() {
  ...
  const s = {
    ...
    navItem: (a) => ({ display: "flex", alignItems: "center", gap: 10, padding: "10px 20px", cursor: "pointer", color: a ? "#e8edf5" : "#6b7a8d", background: a ? "rgba(255,255,255,0.04)" : "transparent", fontSize: 13, fontWeight: a ? 600 : 400, borderLeft: a ? "2px solid #a78bfa" : "2px solid transparent" }),
  ...
  return (
    <div style={s.root}>
      ...
      <div style={s.sidebar}>
        ...
        <div style={{ padding: "0 20px 28px", cursor: "pointer" }} onClick={() => navigate("/")}>
          <LogoImg size={36} />
        </div>
        <div style={{ borderTop: "1px solid #181f2a", paddingTop: 20 }}>
          ...
          {NAV_ITEMS.map(item => (
            <div key={item.label} style={s.navItem(item.active)}
              ...
              <span style={{ width: 22, height: 22, borderRadius: "50%", border: `1px solid ${item.active ? "#a78bfa" : "#2a3a4a"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: item.active ? "#a78bfa" : "#3a4555", flexShrink: 0 }}>{item.icon}</span>
      ...
      <div style={s.main}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 22 }}>
            {["♡", "✈", "🎓", "⏩"].map((ic, i) => (
              <span key={i} style={{ color: "#4a5568", fontSize: 16, cursor: "pointer" }}
                ...
                onMouseLeave={e => e.target.style.color = "#4a5568"}>{ic}</span>
            ))}
          </div>
        ...
        <div style={s.content}>
          ...
          <div style={{ ...s.card, padding: "16px 20px" }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
              ...
              <button onClick={() => setLaunchpadExpanded(v => !v)}
        ...
        {/* Footer */}
        <div style={s.footer}>
          <div style={{ fontSize: 18, fontWeight: 800, color: "#e8edf5" }}>Dot<span style={{ color: "#a78bfa" }}>IPA</span>D</div>
